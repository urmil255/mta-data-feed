...
const CurrentOutages = () => {
  ...
  useEffect(() => {
    ...
    const apiEndpoint = 'https://api-endpoint.mta.info/Dataservice/mtagtfsfeeds/nyct%2Fnyct_ene.xml';
    fetch(apiEndpoint)
  ...
  const handleSelect = (event, outage) => {
    const selectedIndex = selectedOutages.indexOf(outage);
    let newSelected = [];
    if (selectedIndex === -1) {
      newSelected = newSelected.concat(selectedOutages, outage);
    } else {
      newSelected = newSelected.filter(o => o !== outage);
    }
